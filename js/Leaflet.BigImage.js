...
(function (factory, window) {
    ...
    L.Control.BigImage = L.Control.extend({
        ...
        onAdd: function (map) {
            ...
            var title = this.options.printControlTitle;
            var label = this.options.printControlLabel;
        ...
        _print: function () {
            ...
            promise.then(() => {
                return new Promise(((resolve, reject) => {
                    for (var [key, layer] of Object.entries(self.tilesImgs)) {
                        for (var [key, value] of Object.entries(layer)) {
                            self.ctx.drawImage(value.img, value.x, value.y, self.tileSize, self.tileSize);
                        }
                    }
                    for (var [key, value] of Object.entries(self.path)) {
                        self._drawPath(value);
                    }
                    for (var [key, value] of Object.entries(self.markers)) {
                        self.ctx.drawImage(value.img, value.x, value.y);
                    }
                    for (var [key, value] of Object.entries(self.circles)) {
